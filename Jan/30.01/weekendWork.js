...
const salaryC = (name, salary, ...kids) => {
  ...
  let results;
  if (kids.length == 0) {
    netto = salary - salary * 0.55;
  }
  if (kids.length == 1) {
    netto = salary - salary * 0.3;
  }
  if (kids.length == 2) {
    netto = salary - salary * 0.25;
  }
  if (kids.length > 2) {
  ...
  let bankChecker = num
    ...
    .find(
      (bank) =>
        bank.slice(0, 2) == "DE" &&
        bank.length == 22 &&
        bank.slice(3, 22) == parseInt(bank.slice(3, 22))
    );
  if (bankChecker == undefined) {
...
const cheackMonth = (str) => {
  ...
  let findIt = months.find(
    (month) => month == str.split(" ").find((m) => m == month)
  );
  let season;
  if (findIt == undefined) {
    return `I couldn’t find any month in your text, sorry try again`;
  }
  ...
  if (currentS >= 3 && currentS < 6) {
    season = seasons[1];
  }
  if (currentS >= 6 && currentS < 9) {
    season = seasons[2];
  }
  if (currentS >= 9 && currentS < 12) {
    season = seasons[3];
  }
...
let monthsR = months.filter((month) => month.includes("r"));
...
let evenN = num.filter((even) => even % 2 == 0);
console.log(evenN);
console.log(
  "    ---------------------         break        --------------               "
);
...
let testMap = companies.map(
  (company) =>
    `the is ${company.category} company is ${
...
let olderThan = ages.filter((num) => num > 18 && num < 30);
